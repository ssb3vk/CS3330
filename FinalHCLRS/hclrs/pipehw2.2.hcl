...
wire loadUse:1, conditionsMet:1; 
...
conditionsMet = [ #checking for JXX mispredictions
...
stall_F = loadUse; 
stall_D = loadUse; 
bubble_D = conditionsMet || loadUse;
...
reg_srcA = [
	D_icode in {POPQ, RET} : REG_RSP; # This is for POPQ read location logic (normally POPQ in below cond)
	D_icode in {RRMOVQ, RMMOVQ, OPQ, PUSHQ, CALL} : D_rA; # do I need call here? 
	1 : REG_NONE;
];
reg_srcB = [
	D_icode in {RMMOVQ, MRMOVQ, OPQ} : D_rB;
	D_icode in {PUSHQ, POPQ, CALL, RET, CALL} : REG_RSP; 
	1 : REG_NONE;
];
...
d_valA = [
	...
	reg_srcA == m_dstE : m_valE; #do we need these for pushq?
...
d_valB = [	
	...
	reg_srcB == m_dstE : m_valE; #do we need these for pushq?
...
d_dstM = [
	d_icode in {MRMOVQ, PUSHQ, POPQ, RET} : D_rA; #used for POPQ and RET? 
	1 : REG_NONE; 
]; 
...
ALUB = [
	...
	E_icode in { PUSHQ, POPQ, CALL, RET} : E_valB; # needed for PUSH/POP
	1: E_valB;
];
...
e_valA = [ # is this the right way to go about implementing POPQ?
	e_icode in {POPQ, RET} : ALUB; 	
	1 : E_valA;  
]; 
...
mem_addr = [
	M_icode == POPQ : M_valA; # normally popq in below operation
...
f_predPC = [ #page 449 textbook
	f_icode in {JXX, CALL, RET} : f_valC; 
	1 : f_valP;
]; 
